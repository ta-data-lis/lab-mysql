#INSERT INTO class.students (student_id, student_name, coolness)
 #VALUES (100, 'Fred Raposo', 1);
 
 SET sql_mode = "NO_AUTO_VALUE_ON_ZERO";
 
 ALTER TABLE lab_mysql.cars AUTO_INCREMENT = 0;
 
 INSERT INTO lab_mysql.cars (vin, manufacturer, model, year, color)
 VALUES ( "3K096I98581DHSNUP", "Volkswagen", "Tiguan", 2019, "Blue"),
		( "ZM8G7BEUQZ97IH46V", "Peugeot", "Rifter", 2019, "Red"),
		( "RKXVNNIHLVVZOUB4M", "Ford",	"Fusion",	2018,	"White"),
		( "HKNDGS7CU31E9Z7JW", "Toyota",	"RAV4",	2018,	"Silver"),
		( "DAM41UDN3CHU2WVF6", "Volvo",	"V60",	2019,	"Gray"),
		( "DAM41UDN3CHU2WVF6", "Volvo", "V60 Cross Country", 2019, "Gray");
        
        
SELECT * FROM lab_mysql.cars;

SET sql_mode = "NO_AUTO_VALUE_ON_ZERO";
 INSERT INTO lab_mysql.customers ( customer_id, cust_name, phone_nr, email, address, city, state, country, zip)
 VALUES (	10001, "Pablo Picasso", "+34636176382", "" ,"Paseo de la Chopera, 14",	"Madrid", "Madrid",	"Spain", 28045),
		(	20001, "Abraham Lincoln", "+13059077086", "" ,"120 SW 8th St", "Miami", "Florida", "United States",	33130),
		(	30001, "Napoléon Bonaparte", "+33179754000", "" ,"40 Rue du Colisée",	"Paris", "Île-de-France", "France", 75008);
        
SELECT * FROM lab_mysql.customers;

 INSERT INTO lab_mysql.salesperson ( staff_id, staff_name, store)
 VALUES (	"00001",	"Petey Cruiser",	"Madrid"),
		(	"00002",	"Anna Sthesia",	"Barcelona"),
		(	"00003",	"Paul Molive",	"Berlin"),
		(	"00004",	"Gail Forcewind",	"Paris"),
		(	"00005",	"Paige Turner",	"Mimia"),
		(	"00006",	"Bob Frapples",	"Mexico City"),
		(	"00007",	"Walter Melon",	"Amsterdam"),
		(	"00008",	"Shonda Leer",	"São Paulo");
        
SELECT * FROM lab_mysql.salesperson;

 INSERT INTO lab_mysql.invoices (invoice_nr, inv_date, car, customer, salesperson)
 VALUES ("852399038", STR_TO_DATE('22-08-2018', '%d-%m-%Y'), 0,	1,	3),
		("731166526", STR_TO_DATE('31-12-2018', '%d-%m-%Y'), 3, 0, 5),
		("271135104", STR_TO_DATE('22-01-2019', '%d-%m-%Y'), 2, 2, 7);
        
SELECT * FROM lab_mysql.invoices;